...
def gen_image(num_images, h1_mask, h2_mask, out_m, dir_m):
    ...
    #iterate time
    h1_mask = tf.cast(h1_mask, float32)
    h2_mask = tf.cast(h2_mask, float32)
    out_m = tf.cast(out_m, float32)
    dir_m = tf.cast(dir_m, float32)
    w1 = tf.cast(w1, float32)
    b1 = tf.cast(b1, float32)
    w2 = tf.cast(w2, float32)
    b2 = tf.cast(b2, float32)
    x_b_hat = tf.cast(x_b_hat, float32)
    x_hat = tf.cast(x_hat, float32)
